...
Advanced Google Keys mod by miz
By MIZUNUMA Yuto — Last update Sep 29, 2011 — Installed 81 times.
About
Source Code
Reviews 0
Discussions 0
...
window.addEventListener("load", function(e) {
    ...
    for(i=0, j=0, k=0;k<tmp.childNodes.length;k++){
        t = tmp.childNodes.item(k+1);
        if(tmp.childNodes.item(k+1) && tmp.childNodes.item(k+1).tagName == 'LI'){
            results[i++] = tmp.childNodes.item(k+1);
//            if(tmp.childNodes.item(k+1).className=="g w0") {
            c = tmp.childNodes.item(k+1).className;
            id = tmp.childNodes.item(k+1).id;
            if(c =="g w0" || c == "g" || c == "g w5") {
//            if(c =="g w0" || (c == "g" && id != "productbox") || c == "g w5") {
//                GM_log("id:" + id);
                if(t.firstChild.className == "vsc"){
                    resultsForNumber[j++] = tmp.childNodes.item(k+1);
                }
//                GM_log(tmp.childNodes.item(k+1).innerHTML);
            }
        }
    }
    ...
    // if(window.document.getElementById('imagebox')){
    //     imageBox =window.document.getElementById('imagebox').firstChild.firstChild;
    //     tmp = window.document.getElementById('imagebox').firstChild;
    //     tmp.innerHTML="<font class='key_accs' color=#ffffff style='background:"+color+"'>"
    //                 +"&nbsp;"+ 'I' +"&nbsp;</font> "+tmp.innerHTML;
    // }
